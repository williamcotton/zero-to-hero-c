...
/**
 * Writes a CSV file for use with gnuplot commands later.  Allows files to also
 * be saved for post analysis with excel for example. Arguments are similar to
 * gnuplot_plot_x()
 *
 ...
 *
 * @param fileName file name to write to.  This should be the same name used in
 * the gnuplot command
 * @param d
 * @param n
 * @param title
...
/**
 * Writes a multi column CSV file for use with gnuplot commands later.  Allows
 * files to also be saved for post analysis with excel for example. Note that
 * when used with gnuplot, since there may be more than one column the whole
 * "1:3" or whatever should be used.
 *
 * Uses title as gnuplot "comment" on the first line.
 *
 * @author Peter H Maresh 11/8/2011
 *
 * @param fileName      file name to write to.  This should be the same name
 * used in the gnuplot command
 * @param xListPtr      A list of pointers to column buffers
 * @param n
 * @param numColumns    Length of xListPtr list.  Note that the CSV file will
 * add an additional "index" column first.
 * @param title         Title to write for the first line of the .csv file, will
 * be preceeded by
 *                      "#"
 * @return int <0 if file wasn't written.
 */
